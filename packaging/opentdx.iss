; ===========================================================================
; OpenTDX Stock Panel — Inno Setup 安装包脚本
; ===========================================================================
; 用途: 把 PyInstaller 产出的 dist/OpenTDXStockPanel/ 文件夹封装成
;       单个 Setup.exe 安装程序 (双击→安装向导→快捷方式→可卸载)。
;
; 构建 (本地):
;   1. 先跑 PyInstaller: cd backend && uv run pyinstaller ../packaging/opentdx.spec
;   2. 再跑 Inno Setup:   ISCC.exe packaging\opentdx.iss
;   3. 产物: packaging\Output\OpenTDXStockPanel-Setup-x.x.x.exe
;
; 设计决策:
...
#define MyAppName          "OpenTDX 股票面板"
#define MyAppNameEN       "OpenTDX Stock Panel"
#define MyAppExeName      "OpenTDXStockPanel.exe"
#define MyAppPublisher    "OpenTDX"
...
DefaultDirName=D:\OpenTDXStockPanel
DefaultGroupName={#MyAppName}
DisableProgramGroupPage=yes
OutputDir=Output
OutputBaseFilename=OpenTDXStockPanel-Setup-{#MyAppVersion}
...
Source: "..\backend\dist\OpenTDXStockPanel\*"; DestDir: "{app}"; Flags: ignoreversion recursesubdirs createallsubdirs
...
begin
  ...
  begin
    DefaultDir := ExpandConstant('{localappdata}\Programs\OpenTDXStockPanel');
