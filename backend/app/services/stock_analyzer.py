...
- **明确操作建议**:必须结合用户当前仓位。已持仓时给出持有/加仓/减仓/止损的仓位动作;未持仓时给出观察/试仓/放弃的触发条件。再分别说明激进型 / 稳健型 / 保守型 怎么应对
...
_OFFICIAL_SYSTEM_PROMPT = """你正在接收 OpenTDX Stock Panel 提供的结构化 A 股上下文。请优先使用你已内置的金融分析能力与可用 skill 进行分析,不要被项目侧重复的分析框架限制。

硬性要求:
1. 只基于用户消息中提供的行情、关键价位、财务与持仓数据推理;缺失数据必须说明缺失,不得编造。
2. 输出 Markdown 正文,面向交易决策,结论要可执行。
3. 若存在持仓上下文,必须结合仓位、成本、浮盈亏、现金和总仓位给出持有/加仓/减仓/止损/止盈等操作建议。
4. 报告末尾附风险提示:本报告仅供参考,不构成投资建议。"""

_PROMPT_PROFILE_GENERIC = "generic"
_PROMPT_PROFILE_OFFICIAL = "official"


def _current_prompt_profile() -> str:
    from app.services.ai_provider import is_official_llm_provider

    return _PROMPT_PROFILE_OFFICIAL if is_official_llm_provider() else _PROMPT_PROFILE_GENERIC


def _system_prompt_for_profile(profile: str) -> str:
    return _OFFICIAL_SYSTEM_PROMPT if profile == _PROMPT_PROFILE_OFFICIAL else _SYSTEM_PROMPT
...
def _build_user_prompt(
    ...
    symbol: str,
    focus: str,
    prompt_profile: str = _PROMPT_PROFILE_GENERIC,
) -> str:
    ...
    if prompt_profile == _PROMPT_PROFILE_OFFICIAL:
        parts.extend([
            "",
            "以下是用户手工同步的真实账户/仓位上下文(JSON)。请结合该上下文输出仓位相关建议:",
            "```json",
        ...
        parts.extend([
            "",
            "以下是用户手工同步的真实账户/仓位上下文(JSON)。"
            "请把操作建议建立在该上下文上,不要只给泛泛的买卖区间。",
            "若 held=true,必须围绕现有持仓给出持有、加仓、减仓、止损/止盈动作和触发价;",
            "若 held=false 但 has_positions=true,请结合账户整体仓位与现金余量控制新开仓风险;",
            "若 has_positions=false,按空仓用户给出观察/试仓条件:",
            "```json",
            json.dumps(portfolio_context, ensure_ascii=False),
            "```",
        ])
    ...
    if has_fin:
        ...
        if prompt_profile == _PROMPT_PROFILE_OFFICIAL:
            parts.extend([
                "",
                "(该标的暂无财务数据:当前为 Free 模式或尚未同步财务报表。请明确说明财务数据缺失,不要编造财务指标。)",
            ])
        else:
            parts.extend([
                "",
...
def _portfolio_meta(portfolio_context: dict) -> dict:
    """Return a compact, UI-safe portfolio summary for stream metadata."""
    position = portfolio_context.get("position") or None
    ...
    return {
        "held": bool(portfolio_context.get("held")),
        "has_positions": bool(portfolio_context.get("has_positions")),
        "position_count": int(portfolio_context.get("position_count") or 0),
        "position": compact_position,
        "account": {
            "cash": account.get("cash"),
            "market_value": account.get("market_value"),
            "total_assets": account.get("total_assets"),
            "position_ratio": account.get("position_ratio"),
        },
    }
...
async def analyze_stock_stream(
    ...
    portfolio_context = trading_journal.portfolio_context_for_symbol(data_dir, symbol, close)
    ...
    yield json.dumps({
        ...
        "levels": levels,
        "close": close,
        "portfolio": _portfolio_meta(portfolio_context),
    }, ensure_ascii=False)
    ...
    try:
        ...
        prompt_profile = _current_prompt_profile()
        ...
        async for delta in stream_ai_text(
            [
                {"role": "system", "content": _system_prompt_for_profile(prompt_profile)},
